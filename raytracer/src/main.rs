...
fn ray_color(r: &ray, world: &hittable_list, depth:i32) -> Vec3 {
    ...
    if world.hit(r, 0.001, infinity, &mut rec) {
        ...
        if rec.mat_ptr.clone().unwrap().scatter(r, &mut rec, &mut attenuation, &mut scattered) {
...
fn random_scene() -> hittable_list{
    ...
    let mut ground_material: Option<Arc<dyn material>> = Some(Arc::new(lambertian::new(&Color1::new(0.5,0.5,0.5))));
    ...
    let material2: Option<Arc<dyn material>> = Some(Arc::new(lambertian::new(&Color1::new(0.4,0.2,0.1))));
    world.add(Some(Arc::new(sphere::new(
        Point3::new(-4.0,1.0,0.0), 1.0, material2
    ))));

    let material3: Option<Arc<dyn material>> = Some(Arc::new(medal::new(&Color1::new(0.7,0.6,0.5), 0.0)));
    world.add(Some(Arc::new(sphere::new(
